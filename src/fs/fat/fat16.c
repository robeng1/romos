...
struct fat_h
{
  struct fat_header primary_header; // Primary header
  union fat_h_e
  {
    struct fat_header_extended extended_header; // Extended header
  } shared;
};
...
{
  struct fat_directory_item *item; // Pointer to directory items
...
struct fat_item
{
  union
  {
    struct fat_directory_item *item; // Pointer to a directory item (if this is a file)
    struct fat_directory *directory; // Pointer to a directory (if this is a directory)
  };
...
{
  struct fat_item *item; // Pointer to the associated FAT item
...
struct fat_private
{
  struct fat_h header;                 // FAT16 header
  struct fat_directory root_directory; // Root directory
