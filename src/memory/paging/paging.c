#include "paging.h"
#include "memory/heap/kernel_heap.h" // Include for memory allocation functions
...
int paging_map_to(struct paging_4GB_chunk *directory, void *virt, void *phys, void *phys_end, int flags)
{
  ...
  if ((uint32_t)phys % PAGING_PAGE_SIZE || (uint32_t)phys_end % PAGING_PAGE_SIZE)
  {
    // If not aligned, return an error code
    res = -EINVARG;
    goto out;
...
void *paging_get_physical_address(uint32_t *directory, void *virt)
{
  ...
  uint32_t phys = table[table_index] & 0xfffff000;

  // Calculate the physical address by adding the page offset
  return (void *)(phys + ((uint32_t)virt & 0x00000fff));
}
