...
struct disk_stream *new_disk_stream(int disk_id) // Function to create a new disk stream
{
  struct disk *disk = disk_get(disk_id); // Get the disk corresponding to the provided disk ID
  if (!disk)                             // If disk retrieval fails (disk is not found)
  {
    return 0; // Return NULL to indicate failure
  }

  struct disk_stream *stream = kernel_zalloc(sizeof(struct disk_stream)); // Allocate memory for a new disk stream structure
...
int disk_stream_seek(struct disk_stream *stream, int pos) // Function to set the position of the disk stream
{
  stream->pos = pos; // Set the stream position to the provided position
  return 0;          // Return 0 to indicate success
}

int disk_stream_read(struct disk_stream *stream, void *out, int total) // Function to read data from the disk stream
{
...
void disk_stream_close(struct disk_stream *stream) // Function to close the disk stream
{
